...
bool touchZonePressed(String zone) {
  ...
  // Adjust these based on your screen rotation
...
//float currentBatteryVoltage = 0.0;  // Safe default
...
if (tft.getTouch(&x, &y) && !feature_active && (millis() - lastTouchTime >= touchFeedbackDelay)) {
    lastTouchTime = millis();  // Update touch timestamp
    ...
    for (int i = 0; i < NUM_MENU_ITEMS; i++) {
        ...
        if (x >= button_x1 && x <= button_x2 && y >= button_y1 && y <= button_y2) {
            ...
            while (tft.getTouch(&x, &y) && (millis() - startTime < touchFeedbackDelay)) {
                delay(10); 
            }

            // Check if touch is still active after release delay
            if (tft.getTouch(&x, &y)) {
                updateActiveSubmenu(); 
...
void launchReplayAttack() {
  ...
  tft.println("Launching Replay Attack...");
  delay(500);
  ...
  while (!feature_exit_requested) {
    ...
    if (ts.touched()) {
      waitForTouchRelease();  // Optional helper for debounce
  ...
  tft.println("Exiting Replay Attack...");
  delay(500);
...
void loop() {
  ...
  // Manage backlight based on user activity
  manageBacklight();
  ...
  if (tft.getTouch(&x, &y)) {
    ...
    tft.fillCircle(x, y, 3, TFT_GREEN);  // Visual feedback
  ...
  // You can add menu/submenu handling here later if needed
}
